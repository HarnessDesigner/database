"""cpa_locks table -- ported from harness_designer/database/create_database/cpa_locks.py.
...
_log = logging.getLogger('builder.cpa_locks')
...
def add_cpa_lock(con, part_number, description, mfg=None, family=None, series=None,
    ...
    _log.debug('cpa lock added %r -> %s', part_number, new_id.hex())
...
def add_records(con, data_path):
    con.execute('SELECT 1 FROM cpa_locks LIMIT 1;')
    if con.fetchall():
        return
    ...
    for name, path in dirs:
        ...
        if os.path.exists(json_path):
            ...
            data_len = len(data)
            _log.info('adding %d %scpa lock(s) to db', data_len, name)
            ...
            types_mapping = _insert_data(con, types, 'cpa_lock_types', 'name')
            ...
            con.executemany('INSERT INTO cpa_locks (id, part_number, description, mfg_id, family_id, '
                            'series_id, color_id, image_id, datasheet_id, cad_id, min_temp_id, '
                            'max_temp_id, model3d_id, type_id, length, width, height, weight, pins, '
                            'terminal_size, compat_housings) '
                            'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);',
