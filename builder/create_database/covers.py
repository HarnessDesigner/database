"""covers table -- ported from harness_designer/database/create_database/covers.py.

Same generic add_records + single Bosch hook pattern as housings.py -- see
that file's module docstring and builder/bosch_models.py for why.

Fixes a copy-paste bug in the original DDL: `datasheet_id`'s SQLFieldReference
pointed at `_directions.id_field` instead of `_datasheets.id_field` (wrong
field object from the wrong table, same `table=_con.SQLTable` name coincidence
masked it). Corrected here.
"""
...
_log = logging.getLogger('builder.covers')
...
def add_cover(con, part_number, description, mfg=None, family=None, series=None,
    ...
    _log.debug('cover added %r -> %s', part_number, new_id.hex())
...
def add_records(con, data_path):
    con.execute('SELECT 1 FROM covers LIMIT 1;')
    if con.fetchall():
        return
    ...
    for name, path in dirs:
        ...
        if os.path.exists(json_path):
            ...
            data_len = len(data)
            _log.info('adding %d %scover(s) to db', data_len, name)
            ...
            for item in data:
                ...
                row = [new_id, part_number, description,
                       mfg, family, series, color_id, direction, image, datasheet, cad,
                       min_temp_id, max_temp_id, model3d,
                       length, width, height, weight, pins, compat_housings]
            ...
            for item in new_data:
                (mfg, family, series, direction, image, datasheet, cad, model3d) = (
                    item[3], item[4], item[5], item[7], item[8], item[9], item[10], item[13])
            ...
            con.executemany('INSERT INTO covers (id, part_number, description, mfg_id, family_id, '
                            'series_id, color_id, direction_id, image_id, datasheet_id, cad_id, '
                            'min_temp_id, max_temp_id, model3d_id, length, width, height, weight, '
                            'pins, compat_housings) '
                            'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);',
